...
internal sealed class McpRequestServerAccessor
{
	...
	/// <summary>
	/// Whether the flowing request belongs to a revision that carries its fields per request rather
	/// than establishing a session.
	/// </summary>
	public bool IsSessionlessRequest =>
		McpProtocolRevisions.CarriesSessionlessFields(
			(_current.Value?.JsonRpcMessage as JsonRpcRequest)?.Context?.ProtocolVersion);
}
