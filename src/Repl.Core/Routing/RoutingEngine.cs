...
/// <summary>
/// Encapsulates route registration, prefix resolution, context validation, and banner rendering
/// previously hosted inside <c>CoreReplApp.Routing.cs</c>.
/// </summary>
internal sealed class RoutingEngine(CoreReplApp app)
{
	internal ContextDefinition RegisterContext(string template, Delegate? validation, string? description)
	{
		var parsedTemplate = RouteTemplateParser.Parse(template, app.OptionsSnapshot.Parsing);
		var moduleId = app.ResolveCurrentMappingModuleId();
	...
	{
		var contextMatches = ContextResolver.ResolvePrefixes(contexts, matchedPathTokens, app.OptionsSnapshot.Parsing);
	...
	private IReadOnlyList<ContextMatch> ResolveRouteContextPrefixes(
		...
		IReadOnlyList<ContextDefinition> contexts)
	{
		var matches = ContextResolver.ResolvePrefixes(contexts, matchedPathTokens, app.OptionsSnapshot.Parsing);
		return [..
	...
	internal static IReplResult CreateAmbiguousPrefixResult(PrefixResolutionResult prefixResolution)
	{
	...
	{
		if (app.AllBannersSuppressed.Value || !app.OptionsSnapshot.Output.BannerEnabled)
		{
			return false;
		}

		var format = string.IsNullOrWhiteSpace(requestedOutputFormat)
			? app.OptionsSnapshot.Output.DefaultFormat
			: requestedOutputFormat;
		return app.OptionsSnapshot.Output.BannerFormats.Contains(format);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		{
			var styled = app.OptionsSnapshot.Output.IsAnsiEnabled()
				? AnsiText.Apply(text, app.OptionsSnapshot.Output.ResolvePalette().BannerStyle)
				: text;
			await ReplSessionIO.Output.WriteLineAsync(styled).ConfigureAwait(false);
		}
	}

	internal string BuildBannerText()
	{
		...
		for (var i = 0; i < template.Segments.Count; i++)
		{
			...
			switch (segment)
			{
				...
				case DynamicRouteSegment dynamic
					when !RouteConstraintEvaluator.IsMatch(dynamic, scopeToken, app.OptionsSnapshot.Parsing):
					return false;
			}
		}
	...
	private IReadOnlyList<string> GetCandidateLiterals(PrefixTemplate template, string[] tokens, int index)
	{
		...
		for (var i = 0; i < index; i++)
		{
			...
			if (segment is DynamicRouteSegment dynamic
				&& !RouteConstraintEvaluator.IsMatch(dynamic, token, app.OptionsSnapshot.Parsing))
			{
				return [];
			}
		}
	...
	private static string? FindBestSuggestion(string input, string[] candidates)
	{
		...
		{
			var distance = CoreReplApp.ComputeLevenshteinDistance(
				normalizedInput,
